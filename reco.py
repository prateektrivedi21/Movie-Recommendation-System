        ...
        with cols[idx % num_columns]:
            st.markdown(
                ...
                <div class='scrollable-text' title='{recommended_movie_names[idx]}'>
                 <h5 class='movie-text'>{recommended_movie_names[idx]}<br><h6 class='movie-text'>Released on: {datetime.strptime(recommended_movie_release_dates[idx], '%Y-%m-%d').strftime('%b %d, %Y')}</h6></h5>
                </div>
                """,
                unsafe_allow_html=True
            )
